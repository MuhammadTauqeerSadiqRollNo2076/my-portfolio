        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 max-w-7xl mx-auto">
          ...
          >
            ...
            <div className="bg-white/80 dark:bg-gray-800/80 rounded-2xl p-5 sm:p-6 backdrop-blur-sm border border-gray-200 dark:border-gray-700 shadow-lg">
              <h3 className="text-xl sm:text-2xl font-bold text-gray-800 dark:text-white mb-4 sm:mb-6 flex items-center gap-3">
                <MessageCircle className="text-indigo-500" size={24} />
                Get In Touch
              </h3>
                ...
                {contactInfo.map((item, index) => {
                  ...
                  return (
                    <motion.a
                      ...
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: 0.8 + index * 0.1 }}
                      className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 p-4 rounded-xl bg-gray-50 dark:bg-gray-700/50 hover:bg-indigo-50 dark:hover:bg-gray-700 transition-all duration-300 group cursor-pointer"
                    >
                      <div className="p-2 bg-indigo-100 dark:bg-indigo-900/30 rounded-lg group-hover:bg-indigo-200 dark:group-hover:bg-indigo-900/50 transition-colors">
                        <Icon size={20} className="text-indigo-600 dark:text-indigo-400" />
                      </div>
                      <div className="flex-1">
                        ...
                        <p className="text-gray-600 dark:text-gray-300 font-medium break-words">{item.value}</p>
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{item.description}</p>
                      </div>
                    </motion.a>
                  );
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {socialLinks.map((social, index) => (
                  <motion.a
                    ...
                    transition={{ delay: 1.4 + index * 0.1 }}
                    whileHover={{ scale: 1.05 }}
                    className={`bg-gradient-to-r ${social.color} text-white py-2 sm:py-3 px-2 rounded-xl text-center text-sm sm:text-base font-semibold shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center gap-1 sm:gap-2`}
                  >
          ...
          >
            {/* Form Tabs */}
            <div className="flex flex-col sm:flex-row gap-2 mb-6 bg-white/80 dark:bg-gray-800/80 rounded-2xl p-2 backdrop-blur-sm border border-gray-200 dark:border-gray-700 shadow-lg">
              {[
                { id: "form", label: "Contact Form", icon: Mail },
                { id: "quick", label: "Quick Chat", icon: MessageCircle },
              ].map((tab) => {
                ...
                return (
                  <button
                    key={tab.id}
                    onClick={() => setActiveTab(tab.id)}
                    className={`flex items-center justify-center gap-2 flex-1 px-4 py-3 rounded-xl font-semibold text-sm sm:text-base transition-all duration-300 ${
                      activeTab === tab.id
                        ? "bg-gradient-to-r from-indigo-500 to-purple-600 text-white shadow-lg"
                        : "text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white"
                    }`}
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5 }}
              className="bg-white/80 dark:bg-gray-800/80 rounded-2xl shadow-2xl p-6 sm:p-8 backdrop-blur-sm border border-gray-200 dark:border-gray-700"
            >
              <div className="flex flex-col sm:flex-row items-center justify-center mb-6 text-indigo-600 dark:text-indigo-400">
                <Mail className="w-8 h-8 mb-2 sm:mb-0 sm:mr-3" />
                <h3 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent text-center">
                  Send Message
                </h3>
              </div>
              ...
              {activeTab === "form" ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600 dark:text-gray-300 mb-6 max-w-md mx-auto px-2">
                    Prefer instant communication? Reach out directly through any of my social platforms for a faster response.
                  </p>
                  <motion.a
                    href="#"
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    className="inline-flex items-center gap-2 px-5 sm:px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl font-semibold shadow-lg hover:shadow-green-500/25 transition-all duration-300 text-sm sm:text-base"
                  >
                    Start Quick Chat
                    <ExternalLink size={18} />
                  </motion.a>
              ...
              >
                <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-3 text-xs sm:text-sm text-gray-500 dark:text-gray-400 text-center">
                  <Send className="w-4 h-4 text-indigo-400" />
                  <span>I'm committed to responding within 24 hours</span>
                </div>
              </motion.div>
